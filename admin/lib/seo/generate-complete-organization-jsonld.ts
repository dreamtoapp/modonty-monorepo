...
export function generateCompleteOrganizationJsonLd(
  ...
  if (Array.isArray(client.knowsLanguage) && client.knowsLanguage.length > 0) {
    organizationNode.knowsLanguage = client.knowsLanguage;
  }
  ...
  if (client.commercialRegistrationNumber) {
    ...
    // Add ISO6523Code format: "0199:SA{CR_NUMBER}"
  ...
  // Enhanced primary image with validation
  if (client.ogImageMedia?.url) {
    const ogImageUrl = ensureAbsoluteUrl(client.ogImageMedia.url, siteUrl) || client.ogImageMedia.url;
    ...
    webPageNode.primaryImageOfPage = {
      ...
      width: ogWidth,
      height: ogHeight,
      ...(client.ogImageMedia.altText && { caption: client.ogImageMedia.altText }),
    };
  }

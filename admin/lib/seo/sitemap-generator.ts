...
// Article with required fields for sitemap
...
export function generateArticleSitemapEntry(
  ...
  const entry: SitemapEntry = {
    ...
    changefreq: (article.sitemapChangeFreq as SitemapEntry["changefreq"]) || "weekly",
  ...
  if (options?.includeNews && article.datePublished) {
    ...
    if (isRecent) {
      entry.news = {
        ...
        publicationLanguage: article.inLanguage || "ar",
...
export function generateArticleSitemap(
  ...
  for (const article of articles) {
    const entry = generateArticleSitemapEntry(article, siteUrl, options);

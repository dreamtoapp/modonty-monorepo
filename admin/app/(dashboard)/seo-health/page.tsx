  ...
  return (
    <div className="container py-8 space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">متوسط وقت التوليد</CardTitle>
            <Clock className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{data.jsonLdStats.avgGenerationTimeMs}ms</div>
            <p className="text-xs text-muted-foreground mt-2">
              {data.jsonLdStats.avgGenerationTimeMs < 100 ? (
                <span className="text-green-500">✓ ضمن الميزانية (100ms)</span>
              ) : (
                <span className="text-red-500">✗ يتجاوز الميزانية</span>
              )}
            </p>
          </CardContent>
        </Card>

...
export function AuthorTable({ authors, onSelectionChange }: AuthorTableProps) {
  ...
  const filteredData = useMemo(() => {
    let result = authors.filter((author) => {
      ...
      return (
        ...
        author.slug.toLowerCase().includes(searchTerm) ||
        (author.jobTitle && author.jobTitle.toLowerCase().includes(searchTerm))
      );
    });
    ...
    return result;
  }, [authors, search, sortKey, sortDirection]);
  ...
  const allSelected = paginatedData.length > 0 && paginatedData.every((author) => selectedIds.has(author.id));
  const someSelected = paginatedData.some((author) => selectedIds.has(author.id));

  const handleSelectAll = (checked: boolean) => {
    const newSelected = new Set(selectedIds);
    if (checked) {
      paginatedData.forEach((author) => newSelected.add(author.id));
    } else {
      paginatedData.forEach((author) => newSelected.delete(author.id));
    }
    setSelectedIds(newSelected);
    onSelectionChange?.(Array.from(newSelected));
  };

  const handleSelectOne = (authorId: string, checked: boolean) => {
    const newSelected = new Set(selectedIds);
    if (checked) {
                ...
                >
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      onChange={(e) => {
                        e.stopPropagation();
                        handleSelectOne(author.id, e.target.checked);
                      }}
                      className="h-4 w-4 rounded border-gray-300"
                    />
                  </TableCell>
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <SEOHealthGauge data={author} config={authorSEOConfig} size="xs" />
                  </TableCell>
                  <TableCell>
                    <Link
                  ...
                  <TableCell>
                    <span className="text-muted-foreground text-sm">{author.slug}</span>
                  </TableCell>
                  <TableCell>{author.jobTitle || "-"}</TableCell>
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Badge variant="secondary">{author._count.articles}</Badge>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <AuthorRowActions authorId={author.id} />
                  </TableCell>
                </TableRow>
              ))
            )}

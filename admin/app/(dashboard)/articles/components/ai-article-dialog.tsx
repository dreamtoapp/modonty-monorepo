...
export function AiArticleDialog({ open, onOpenChange }: AiArticleDialogProps) {
  ...
  const handleConfirm = () => {
    ...
    updateFields({
      ...
      authorId, // Ensure author is set
      faqs: generatedData.faqs.map((faq: any, index: number) => ({
        question: faq.question,
        answer: faq.answer,
        position: index,
      })),
      // Tags are not populated - user will select manually
    });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            أدخل الكلمات المفتاحية واختر طول المقال لتوليد محتوى احترافي جاهز للنشر
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          {step === 'input' && (
            <>
              ...
              <Button onClick={handleGenerate} disabled={!keywords.trim() || isGenerating}>
                {isGenerating ? (
                  <>

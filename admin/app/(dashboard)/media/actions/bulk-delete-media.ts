...
export async function bulkDeleteMedia(ids: string[], clientId?: string) {
  try {
    ...
    mediaList.forEach((m) => {
      ...
      } else if (m.logoClient) {
        cannotDelete.push({
          id: m.id,
          filename: m.filename,
          reason: `Used as logo for client '${m.logoClient.name}'`,
        });
      } else if (m.ogImageClient) {
        cannotDelete.push({
          id: m.id,
          filename: m.filename,
          reason: `Used as OG image for client '${m.ogImageClient.name}'`,
        });
      } else if (m.twitterImageClient) {
        cannotDelete.push({
          id: m.id,
          filename: m.filename,
          reason: `Used as Twitter image for client '${m.twitterImageClient.name}'`,
        });
      }
    });
